...
async def seed_demo_data(session: AsyncSession) -> dict[str, int | str]:
    ...
    if not workspace:
        ...
        session.add(
            CreditTransaction(
                ...
                amount=500,
                balance_after=500,
                transaction_metadata={"source": "demo_seed_endpoint"},
            )
        )
    ...
    return {"workspace_id": workspace.id, "created_documents": created_documents, "document_total": len(DEMO_DOCUMENTS)}
